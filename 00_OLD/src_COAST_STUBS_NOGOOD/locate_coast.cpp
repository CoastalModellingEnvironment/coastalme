...
/*
 * TODO
 * 1. Whole grid search, find all cells below or equal this-iter SWL
 * 2. At same time search all 8 neighbours of each below this-iter SWL cell. If any are above this-iter SWL, save cell coords of above-SWL cell in a temp vector
 * 3. Sort this temp vector by coords. Try sort key (x * xMax) + y
 * 4. Go along temp vector: if a coord is not adjacent to (how to test?) previous coord, create new poss-coast vector (this includes first coord), copy all subsequent adjacent coords to this poss-coast vector
 * 5. For each poss-coast vector:
 *    5a. Compare start coord with start coord of every other poss-coast vector: if adjacent then join the two poss-coord vectors
 *    5b. Compare start coord with end coord of every other poss-coast vector: if adjacent then reverse second poss-coast vector then join the two poss-coord vectors (check for loops?)
 * 6. For each poss-coast vector that is left, make this a coast vector and mark on grid
 * */

namespace
{
//===============================================================================================================================
//! Helper function used when sorting coastline co-ordinates into ascending order. If the first argument must be ordered before the second, return true
//===============================================================================================================================
bool bCoastCoordsAscendingXFirst(CGeom2DIPoint const& pPtiLeft, CGeom2DIPoint const& pPtiRight)
{
   if (pPtiLeft.nGetX() < pPtiRight.nGetX())
      return true;
   else if (pPtiLeft.nGetX() > pPtiRight.nGetX())
      return false;
   else
      return pPtiLeft.nGetY() < pPtiRight.nGetY();
}

//===============================================================================================================================
//! Helper function used when sorting coastline co-ordinates into ascending order. If the first argument must be ordered before the second, return true
//===============================================================================================================================
bool bCoastCoordsAscendingYFirst(CGeom2DIPoint const& pPtiLeft, CGeom2DIPoint const& pPtiRight)
{
   if (pPtiLeft.nGetY() < pPtiRight.nGetY())
      return true;
   else if (pPtiLeft.nGetY() > pPtiRight.nGetY())
      return false;
   else
      return pPtiLeft.nGetX() < pPtiRight.nGetX();
}
} // namespace
...
int CSimulation::nLocateSeaAndCoasts(void)
{
   ...
   vector<CGeom2DIPoint> VPtiTemp;
   FindAllCoastCells(&VPtiTemp);



   // Sort this temp vector by coords
   if (m_nYGridSize > m_nXGridSize)
      sort(VPtiTemp.begin(), VPtiTemp.end(), bCoastCoordsAscendingYFirst);
   else
      sort(VPtiTemp.begin(), VPtiTemp.end(), bCoastCoordsAscendingXFirst);

   // LogStream << "##############################################" << endl;
   // for (int n = 0; n < static_cast<int>(VPtiTemp.size()); n++)
   // {
   //    LogStream << "[" << VPtiTemp[n].nGetX() << "][" << VPtiTemp[n].nGetY() << "]" << endl;
   // }
   // LogStream << "##############################################" << endl;

   // Go along temp vector: if a coord is not adjacent to previous coord, create new stub vector (do same for first coord), then copy all subsequent adjacent coords to this stub vector
   int nSize = static_cast<int>(VPtiTemp.size());
   vector<vector<CGeom2DIPoint>> VVPtiAllCoastStub;
   VVPtiAllCoastStub.push_back(vector<CGeom2DIPoint> ());      // Create first stub

   for (int nPos = 0; nPos < nSize-1; nPos++)
   {
       // Append this cell to the latest coast stub
      VVPtiAllCoastStub.back().push_back(VPtiTemp[nPos]);

      if (! bIsAdjacent(&VPtiTemp[nPos], &VPtiTemp[nPos+1]))
         // These two points are not adjacent, so create a new coast stub
         VVPtiAllCoastStub.push_back(vector<CGeom2DIPoint> ());
   }

   // Append the last point
   VVPtiAllCoastStub.back().push_back(VPtiTemp[nSize-1]);

   // LogStream << "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%" << endl;
   // for (int m = 0; m < static_cast<int>(VVPtiAllCoastStub.size()); m++)
   // {
   //    LogStream << "Stub " << m << endl;
   //    for (int n = 0; n < static_cast<int>(VVPtiAllCoastStub[m].size()); n++)
   //       LogStream << "[" << VVPtiAllCoastStub[m][n].nGetX() << "][" << VVPtiAllCoastStub[m][n].nGetY() << "]" << endl;
   // }
   // LogStream << "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%" << endl;
   ...
   int nNumStub = static_cast<int>(VVPtiAllCoastStub.size());
   vector<bool> VbDone(nNumStub, false);
   if (nNumStub > 1)
   {
      bool bChanged;
      do
      {
         ...
         for (int nThisStub = 0; nThisStub < nNumStub; nThisStub++)
         {
            for (int nOtherStub = 0; nOtherStub < nNumStub; nOtherStub++)
            {
               // Don't consider other stubs just dealt with in outer loop, or dealt with previously
               if ((nOtherStub <= nThisStub) || (VbDone[nOtherStub]))
                  continue;

               if (bIsAdjacent(&VVPtiAllCoastStub[nThisStub].back(), &VVPtiAllCoastStub[nOtherStub].front()))
               {
                  LogStream << "Stub " << nThisStub << " (" << VVPtiAllCoastStub[nThisStub].size() << " points), appending stub " << nOtherStub << " (" << VVPtiAllCoastStub[nOtherStub].size() << " points) at [" << VVPtiAllCoastStub[nThisStub].back().nGetX() << "][" << VVPtiAllCoastStub[nThisStub].back().nGetY() << "] - [" << VVPtiAllCoastStub[nOtherStub].front().nGetX() << "][" << VVPtiAllCoastStub[nOtherStub].front().nGetY() << "]" << endl;

                  // Append the second vector to the end of the first vector
                  VVPtiAllCoastStub[nThisStub].insert(VVPtiAllCoastStub[nThisStub].end(), VVPtiAllCoastStub[nOtherStub].begin(), VVPtiAllCoastStub[nOtherStub].end());

                  LogStream << "Stub " << nThisStub << " now has " << VVPtiAllCoastStub[nThisStub].size() << " points, from [" << VVPtiAllCoastStub[nThisStub].front().nGetX() << "][" << VVPtiAllCoastStub[nThisStub].front().nGetY() << "] to [" << VVPtiAllCoastStub[nThisStub].back().nGetX() << "][" << VVPtiAllCoastStub[nThisStub].back().nGetY() << "]" << endl;
                  LogStream << "*******************" << endl;
                  // for (int nPoint = 0; nPoint < static_cast<int>(VVPtiAllCoastStub[nThisStub].size()); nPoint++)
                  //    LogStream << "[" << VVPtiAllCoastStub[nThisStub][nPoint].nGetX() << "][" << VVPtiAllCoastStub[nThisStub][nPoint].nGetY() << "]" << endl;
                  // LogStream << "*******************" << endl;

                  VbDone[nOtherStub] = true;
                  bChanged = true;

                  continue;
               }

               if (bIsAdjacent(&VVPtiAllCoastStub[nThisStub].front(), &VVPtiAllCoastStub[nOtherStub].back()))
               {
                  LogStream << "Stub " << nThisStub << " (" << VVPtiAllCoastStub[nThisStub].size() << " points), prepending stub " << nOtherStub << " (" << VVPtiAllCoastStub[nOtherStub].size() << " points) at [" << VVPtiAllCoastStub[nThisStub].front().nGetX() << "][" << VVPtiAllCoastStub[nThisStub].front().nGetY() << "] - [" << VVPtiAllCoastStub[nOtherStub].back().nGetX() << "][" << VVPtiAllCoastStub[nOtherStub].back().nGetY() << "]" << endl;

                  // Prepend the second vector to the beginning of the first vector
                  VVPtiAllCoastStub[nThisStub].insert(VVPtiAllCoastStub[nThisStub].begin(), VVPtiAllCoastStub[nOtherStub].begin(), VVPtiAllCoastStub[nOtherStub].end());
               ...
               if (bIsAdjacent(&VVPtiAllCoastStub[nThisStub].back(), &VVPtiAllCoastStub[nOtherStub].back()))
               {
                  LogStream << "Stub " << nThisStub << " (" << VVPtiAllCoastStub[nThisStub].size() << " points), reversing then appending stub " << nOtherStub << " (" << VVPtiAllCoastStub[nOtherStub].size() << " points) at [" << VVPtiAllCoastStub[nThisStub].back().nGetX() << "][" << VVPtiAllCoastStub[nThisStub].back().nGetY() << "] - [" << VVPtiAllCoastStub[nOtherStub].back().nGetX() << "][" << VVPtiAllCoastStub[nOtherStub].back().nGetY() << "]" << endl;

                  // Reverse the second vector
                  reverse(VVPtiAllCoastStub[nOtherStub].begin(), VVPtiAllCoastStub[nOtherStub].end());

                  // Append the second vector (reversed) to the end of the first vector
                  VVPtiAllCoastStub[nThisStub].insert(VVPtiAllCoastStub[nThisStub].end(), VVPtiAllCoastStub[nOtherStub].begin(), VVPtiAllCoastStub[nOtherStub].end());
               ...
               if (bIsAdjacent(&VVPtiAllCoastStub[nThisStub].front(), &VVPtiAllCoastStub[nOtherStub].front()))
               {
                  LogStream << "Stub " << nThisStub << " (" << VVPtiAllCoastStub[nThisStub].size() << " points), reversing then prepending stub " << nOtherStub << " (" << VVPtiAllCoastStub[nOtherStub].size() << " points) at [" << VVPtiAllCoastStub[nThisStub].front().nGetX() << "][" << VVPtiAllCoastStub[nThisStub].front().nGetY() << "] - [" << VVPtiAllCoastStub[nOtherStub].front().nGetX() << "][" << VVPtiAllCoastStub[nOtherStub].front().nGetY() << "]" << endl;

                  // Reverse the second vector
                  reverse(VVPtiAllCoastStub[nOtherStub].begin(), VVPtiAllCoastStub[nOtherStub].end());

                  // Prepend the second vector (reversed) to the beginning of the first vector
                  VVPtiAllCoastStub[nThisStub].insert(VVPtiAllCoastStub[nThisStub].begin(), VVPtiAllCoastStub[nOtherStub].begin(), VVPtiAllCoastStub[nOtherStub].end());
   ...
   // 6. For each poss-coast vector that is left, make this a coast vector and mark on grid
   int nColor = 1;
   for (int nStub = 0; nStub < nNumStub; nStub++)
   {
      if (! VbDone[nStub])
      {
         nColor++;
         ...
         LogStream << "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$" << endl;
         LogStream << "Stub " << nStub << " VbDone = " << VbDone[nStub] << " n points = " << VVPtiAllCoastStub[nStub].size() << endl;
         int nX1 = VVPtiAllCoastStub[nStub].front().nGetX();
         int nY1 = VVPtiAllCoastStub[nStub].front().nGetY();
         int nX2 = VVPtiAllCoastStub[nStub].back().nGetX();
         int nY2 = VVPtiAllCoastStub[nStub].back().nGetY();
         LogStream << "[" << nX1 << "][" << nY1 << "] to [" << nX2 << "][" << nY2 << "] = {" << dGridCentroidXToExtCRSX(nX1) << ", " << dGridCentroidYToExtCRSY(nY1) << "} to {" << dGridCentroidXToExtCRSX(nX2) << ", " << dGridCentroidYToExtCRSY(nY2) << "}" << endl;

         for (int nPoint = 0; nPoint < static_cast<int>(VVPtiAllCoastStub[nStub].size()); nPoint++)
         {
            int nX = VVPtiAllCoastStub[nStub][nPoint].nGetX();
            int nY = VVPtiAllCoastStub[nStub][nPoint].nGetY();
            m_pRasterGrid->m_Cell[nX][nY].SetAsCoastline(nColor);
         }
      }
   }
   ...
   // DEBUG CODE ===========================================================================================================
   string strOutFile = m_strOutPath + "is_sea_";
   ...
   for (int nY = 0; nY < m_nYGridSize; nY++)
      for (int nX = 0; nX < m_nXGridSize; nX++)
         pdRaster[n++] = m_pRasterGrid->m_Cell[nX][nY].bIsSea();

   GDALRasterBand* pBand = pDataSet->GetRasterBand(1);
   pBand->SetNoDataValue(m_dMissingValue);
   int nRetn = pBand->RasterIO(GF_Write, 0, 0, m_nXGridSize, m_nYGridSize, pdRaster, m_nXGridSize, m_nYGridSize, GDT_Float64, 0, 0, NULL);
   if (nRetn == CE_Failure)
   return RTN_ERR_UNKNOWN;
   ...
   // DEBUG CODE ===========================================================================================================
   string strOutFile2 = m_strOutPath + "is_coast_";
   strOutFile2 += to_string(m_ulIter);
   strOutFile2 += ".tif";

   GDALDriver* pDriver2 = GetGDALDriverManager()->GetDriverByName("gtiff");
   GDALDataset* pDataSet2 = pDriver2->Create(strOutFile2.c_str(), m_nXGridSize, m_nYGridSize, 1, GDT_Float64, m_papszGDALRasterOptions);
   pDataSet2->SetProjection(m_strGDALBasementDEMProjection.c_str());
   pDataSet2->SetGeoTransform(m_dGeoTransform);
   double* pdRaster2 = new double[m_nXGridSize * m_nYGridSize];

   pDataSet2 = pDriver2->Create(strOutFile2.c_str(), m_nXGridSize, m_nYGridSize, 1, GDT_Float64, m_papszGDALRasterOptions);
   pDataSet2->SetProjection(m_strGDALBasementDEMProjection.c_str());
   pDataSet2->SetGeoTransform(m_dGeoTransform);

   pdRaster2 = new double[m_nXGridSize * m_nYGridSize];
   int nn = 0;
   for (int nY2 = 0; nY2 < m_nYGridSize; nY2++)
      for (int nX2 = 0; nX2 < m_nXGridSize; nX2++)
         pdRaster2[nn++] = m_pRasterGrid->m_Cell[nX2][nY2].nGetCoastline();

   GDALRasterBand* pBand2 = pDataSet2->GetRasterBand(1);
   pBand2 = pDataSet2->GetRasterBand(1);
   pBand2->SetNoDataValue(m_dMissingValue);
   int nRetn2 = pBand2->RasterIO(GF_Write, 0, 0, m_nXGridSize, m_nYGridSize, pdRaster2, m_nXGridSize, m_nYGridSize, GDT_Float64, 0, 0, NULL);
   if (nRetn2 == CE_Failure)
   return RTN_ERR_UNKNOWN;
...
//===============================================================================================================================
//! Finds and marks all cells which have an elevation less than the this-iteration SWL
//===============================================================================================================================
void CSimulation::FindAllSeaCells(void)
{
   // TODO Set up OpenMP for this loop
   for (int nX = 0; nX < m_nXGridSize; nX++)
   {
      for (int nY = 0; nY < m_nYGridSize; nY++)
      {
         ...
         {
            // This cell is less than the this-iteration SWL, so mark it as sea
            m_pRasterGrid->m_Cell[nX][nY].SetSea();

            // Set the sea depth for this sea cell
            m_pRasterGrid->m_Cell[nX][nY].SetSeaDepth();

            // Set the sea cell's landform category
            ...
            // Also sort out the x-y extremities of the sea for the bounding box (used later in wave propagation)
...
//===============================================================================================================================
//! Finds and marks all cells which have at least one non-sea D8 neighbour, and marks these as coast cells
//===============================================================================================================================
void CSimulation::FindAllCoastCells(vector<CGeom2DIPoint>* pVPtiTemp)
{
   // TODO Set up OpenMP for this loop
   for (int nX = 0; nX < m_nXGridSize; nX++)
   {
      for (int nY = 0; nY < m_nYGridSize; nY++)
      {
         ...
         {
            // This is a land cell, so search all D8 neighbours
            if (bAnAdjacentCellIsSea(nX, nY))
            {
               // At least one neighbouring cell is sea, so this must be a coast cell TODO do we need to flag the cell yet?
               // m_pRasterGrid->m_Cell[nX][nY].SetAsCoastline(INT_NODATA);

               // Save the cell coords in the temp vector
               pVPtiTemp->push_back(CGeom2DIPoint(nX, nY));
            }
         }
      }
   }
...
bool CSimulation::bIdentifyPossibleCoastStart(int const nXThis, int const nYThis, int const nXNext, int const nYNext, vector<CGeom2DIPoint>* pV2DIPossibleStartCell)
{
   ...
   bool const bThisCellIsSea = m_pRasterGrid->m_Cell[nXThis][nYThis].bIsSea();
   bool const bNextCellIsSea = m_pRasterGrid->m_Cell[nXNext][nYNext].bIsSea();
...
int CSimulation::nTraceCoastLine(int const nTraceFromStartCellIndex, vector<CGeom2DIPoint> const* pV2DIPossibleStartCell, vector<bool>* pVbTraced, int const nStartEdge, int const nHandedness, int const nStartSearchDirection)
{
   ...
   do
   {
      ...
      bool const bOnCoast = bAnAdjacentCellIsSea(nX, nY);
      if (bOnCoast)
      {
      ...
      if (bIsWithinValidGrid(nXSeaward, nYSeaward))
      {
         ...
         if (! m_pRasterGrid->m_Cell[nXSeaward][nYSeaward].bIsSea())
         {
      ...
      if (bIsWithinValidGrid(nXStraightOn, nYStraightOn))
      {
         ...
         if (! m_pRasterGrid->m_Cell[nXStraightOn][nYStraightOn].bIsSea())
         {
      ...
      if (bIsWithinValidGrid(nXAntiSeaward, nYAntiSeaward))
      {
         ...
         if (! m_pRasterGrid->m_Cell[nXAntiSeaward][nYAntiSeaward].bIsSea())
         {
...
//===============================================================================================================================
bool CSimulation::bAnAdjacentCellIsSea(int const nX, int const nY)
{
   int nXAdj;
   int nYAdj;
   ...
   for (int nSearchDirection = NORTH; nSearchDirection <= NORTH_WEST; nSearchDirection++)
   {
      ...
      case NORTH:
         ...
         {
            if (m_pRasterGrid->m_Cell[nXAdj][nYAdj].bIsSea())
               return true;
         }
         break;
